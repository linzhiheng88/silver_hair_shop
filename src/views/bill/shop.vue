<template>
  <div class="page-wrap">
    <my-header  :breadcrumbList="['商户账单']"></my-header>
...
<script>
  ...
  import shopPayModal from "./components/shopPayModal";
  export default {
    name: 'Users',
    data: function () {
      return {
        ...
        columns: [
          ...
          },
          {
            ...
            render: (h, params) => {
              ...
              if(params.row.type==2&&this.$store.state.userInfo.shop_id<=0){
    ...
    methods: {
      async getShopList(){
        ...
        if(this.keywordObj.weapp_id)weapp=this.keywordObj.weapp_id;
        let shopResult=await this.$store.dispatch('getShop',weapp)
      ...
      onDataLoad: async function (options, callback) {
        ...
        if(this.keywordObj.weapp_id)options.shop_id=this.keywordObj.weapp_id;
        if(this.keywordObj.shop_id)options.shop_id=this.keywordObj.shop_id;
        const result = await this.$request({
          url:'/shopBill/list/'+options.page+'/'+options.pageSize,
          method:'post',
          data:options
        });
        callback(result);
