<template>
  <div>
    <my-header :breadcrumbList="['京东选品']"></my-header>
      ...
      <Card style="margin-top: 15px">
        ...
        <div class="class-two-div">
          <div class="item" v-for="item in classList">
            <div class="name" @click="classClick(item,null)">{{item.name}}</div>
            <div class="value-box">

              <template v-for="sit in item.children">
                <div class="li" @click="classClick(item,ssit)" v-for="ssit in sit.children">
                  {{ssit.name}}
                </div>
              </template>

<!--              <div class="li" @click="classClick(item,sit)" v-for="sit in item.children">-->
<!--                {{sit.name}}-->
<!--              </div>-->
            </div>
          </div>
...
export default {
  ...
  methods: {
    async searchClick(){
      this.keywordObj.catId=null;
      this.keywordObj.cid1=null;

      this.pushClick()
    },
    async pushClick(){

      this.$store.commit('setJdSearchResult')
      await this.$store.commit('setJdKeywordObj',this.keywordObj)
      this.$store.dispatch('getJdSearchResult')
      // let paramsSrt=''
      // if(this.keywordObj.keyword){
      //   paramsSrt+'?keyword='+this.keywordObj.keyword
      // }

      this.$router.push(`/epShop/jdOfferSearch`)
    ...
    classClick(item,sit){
      ...
      let paramsSrt=`?name=${item.name}&pageNum=${item.pageNum}`

      this.keywordObj.keyword=''
      this.keywordObj.catId=item.catId
      this.keywordObj.oneCatName=item.name
      if(sit){
        this.keywordObj.cid1=sit.catId
        this.keywordObj.threeCatName=sit.name
      }else {
        this.keywordObj.cid1=''
      }
      this.pushClick()
      // this.$router.push(`/epShop/jdOfferSearch`)
