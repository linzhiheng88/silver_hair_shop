...
<!--      <img class="icon" mode="aspectFit" :src="logisticsData.source.logo"></img>-->
      <div class="info">
        <div class="name">{{logisticsData.logisticsCompanyName}}</div>
                <div class="number">{{logisticsData.logisticsBillNo}}</div>
...
<!--          </div>-->
<!--        </div>-->
        <div class="li" v-for="(item,index) in logisticsSteps" :key="index">
          <div class="icon">
            <div class="point"></div>
          </div>
          <div class='content'>
...
<script>
  ...
  export default {
    ...
    computed:{
        logisticsSteps:function () {
        return this.logisticsData.logisticsSteps.reverse()
      }
    },
    async mounted() {
      const result = await this.$request({
        url: '/alibaba/getLogisticsTraceInfoBuyerView',
        method: 'post',
        data: {
            alibaba_order_id:this.extra.alibaba_order_id
        }
      })
      this.loading=false
      if (result.code == 200) {
        ...
        this.error='';
        if(result.data.logisticsSteps.length==0){
            this.error='暂无物流信息';
        }
      }else{
