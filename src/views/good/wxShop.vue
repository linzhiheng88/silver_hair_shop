<template>
  <div>
    <my-header :breadcrumbList="['小商店商品管理']"></my-header>
...
export default {
  ...
  methods: {
    ...
    onDataLoad: async function (options, callback) {
      ...
      // });
      const result = await this.$request({
        url:'/wxShop/list/'+options.page+'/'+options.pageSize,
        method:'post',
        data:options
      });
      callback(result);
...
<<<<<<< HEAD
      for(let i in items){
        items[i].images=JSON.parse(items[i].good.images)
        items[i].name=items[i].good.name
      }
=======
